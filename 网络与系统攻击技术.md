...
一些检测手段
	根据操作系统的特征
		Linux内核的特性：正常情况下，只处理本机MAC地址或者以太广播地址的包。在混杂模式下，许多版本的Linux内核只检查数据包中的IP地址以确定是否送到IP堆栈。因此，可以构造无效以太地址而IP地址有效的ICMP ECHO请求，看机器是返回应答包(混杂模式)还是忽略(非混杂模式)。
...
SYN扫描
    ...
    关闭端口：攻击者SYN, 目标主机RST
    优势&弱势：目标主机不会记录未建立连接，较为隐蔽；需要根用户权限构建定制SYN包
```
...
操作系统主动探测技术
    ...
    主动协议栈指纹鉴别
主动协议栈指纹鉴别
    参考文献：Fyodor, Phrack, Remote OS detection via TCP/IP Stack Finger-Printing, 1998.
...
#### 漏洞扫描软件：Nessus <font color="blue">*使用前要更新</font>
```tex
客户端/服务器模式
...
```tex
	...
	当发送ping请求的数据包声称自己的尺寸超过ICMP上限，也就是加载的尺寸超过64K上限时，就会使ping请求接收方出现内存分配错误，导致TCP/IP堆栈崩溃致使接受方宕机。

防御：
	现在所有的标准TCP/IP实现都已能够对付超大尺寸的包，并且大多数防火墙能够自动过滤这些攻击，包括从Windows 98之后的Windows NT(service pack 3之后), linux, Solaris 和Mac OS都具有抵抗一般ping of death攻击的能力。
...
IP分段含有指示该分段所包含的是原包的哪一段的信息，某些TCP/IP包括service pack 4以前的NT 在收到含有重叠偏移的伪造分段时将崩溃。
...
从分布端向受害者目标主机发送的DDoS都是UDP报文，每一个包含4个空字符，这些报文都从一个端口发出，但随机袭击目标主机上的不同端口。目标主机对每一个报文回复一个ICMP Port Unreachable的信息，大量不同主机发来的这些洪水般的报文源源不断，目标主机将很快慢下来，直至剩余带宽变为0。
```
...
​	call：函数调用
​	ret：调用返回
call的时候，将返回地址存入栈中，通过调用ret指令，将当前栈顶存放的内容作为返回的地址从栈中弹出，并放入EIP寄存器
...
如果在用户名输入栏中输入内容使得SQL语句永真并注释掉密码判定语句，则可以在不知道密码的情况下成功登录。如在用户名栏中输入 'or 1=1 -- 后，语句变为了：

```mysql
select * from teacher where tch_num='xxx' or 1=1 -- and tch_pwd='xxx'
```
...
select from * teacher where tch_name like '%a%' union select 1,2,3,4,5,NOW(),NOW(),NOW() -- '
```
...
90年代末，计算机病毒被定义为：<font color="red">经过存储介质和网络进行传播，从一台计算机系统到另外一台计算机系统，未经授权认证破坏计算机系统完整性的程序或代码</font>
...
恶意代码感染系统时，感染的过程大致可以分为：
1. 通过某种途径传播，进入目标系统
2. 通过修改系统设置实现随系统自启动
3. 激活病毒载荷的预定功能，比如：打开后门等待连接、发起DDOS攻击、盗取敏感信息，进行键盘记录、自我复制、加密用户文件等

<font color="red">注意</font>：除引导区病毒外，其他类型的病毒均需在系统中执行病毒代码，才能实现感染系统的目的。对于不同类型的病毒，它们传播、感染系统的方法也有所不同。

传播方式主要有：电子邮件、网络共享、P2P共享、系统及软件漏洞（包括弱口令）、移动磁盘传播
<font color="blue">通过以上途径传播的恶意代码，通常具有很强的传播性</font>
其他常见感染途径：网页感染、与正常软件捆绑、用户直接运行病毒程序、由其他恶意程序下载或释放
<font color="blue">目前大多数的木马、间谍软件等病毒都是通过这几种方式进入系统，它们通常都不具备传播性。</font>
...
信息收集特性
```tex
大多数间谍软件和一些木马都会收集系统中用户的私人信息，特别是各种帐号和密码。收集到的信息通常都会被病毒通过自带的SMTP引擎发送到某个指定的邮箱。
```
...
捆绑程序欺骗
	将木马程序与正常文件捆绑为一个程序
伪装成应用程序扩展组件（白加黑）
	木马为dll或ocx类型文件，挂在一个有名的软件中，如伪装成资源管理器扩展的explorer.dll。
```
...
非授权性：
	一旦控制端和服务端连接后，控制端将享有服务端的大部分操作权限，包括窃取口令、拷贝或删除文件、修改注册表、控制鼠标和键盘、重新启动计算机等。
```
...
3.检查注册表及常见启动位置
4.查找木马进程或文件：kernel32.exe、sysexplr.exe等
5.文件完整性检查：
...
```tex
陌生DLL、缺少数字签名的DLL
与系统进程名称接近或路径不同的DLL（如 Msimg32.dll -> Msing32.dll 等）
```
...
```tex
病毒感染系统后，病毒进程已被系统加载，或是病毒DLL已经载入到正在运行的系统进程中，导致病毒扫描引擎对检测到的文件无法操作
```
...
6. 域控制器用该用户名在 SAM （安全账号管理器）中找到对应密码散列，然后使用这个密码散列来加密challenge
...
<font color="red">针对TCP协议，欺骗过程存在问题：攻击者冒充其他主机向被攻击者发送SYN包时，被攻击者回应的SYN包会回向冒充的IP地址</font>
...
### 三种ARP欺骗方式
1. 伪装源C把自己伪装成局域网内的另一台主机B，在局域网内的主机A广播ARP请求时应答主机A，使得A的ARP表中记录B的IP地址和C的MAC地址，从而使主机A发往主机B的报文都流向主机C
2. 伪装源C把自己伪装成局域网内的另一台主机B，主动向局域网内的主机A发送ARP应答报文，使主机A主动更新其ARP表中的记录，从而使主机A发往主机B的报文都流向主机C
3. 欺骗源把自己（或者其他非网关主机）伪装成网关，向局域网内的目标主机发送ARP应答报文，使得局域网内的主机误以为欺骗源的MAC地址是网关MAC的地址，并将原本应该流向网关的数据都发送到欺骗源。
...
重置入侵连接
关闭未备案的网站
GFW（Great Firewall，我国部署的国家级互联网出口管控系统）发现违规连接后向两端发送 RST 重置，中断通信
